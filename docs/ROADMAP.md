...
| RA6 | **Closeout** — the adapter's topology + signing + dependency-wall + posture ADRs have landed (`docs/adr/0001`–`0005`); RA6 is the closeout gate: the checkpoint-ack probe (RA4) resolution is recorded as an ADR if it lands an ack, the verifier application ROADMAP B2 row derives SHIPPED, and the full suite + conformance round-trip are green. slug:b2-ra-closeout | ADRs `docs/adr/0001`–`0005` landed; RA4 resolved (ack landed or formally deferred); verifier application ROADMAP B2 row derives SHIPPED; full suite + conformance round-trip green | RA1, RA2, RA3, RA4, RA5 | [charter](charter.md) · [strategy](strategy.md) |
