...
homes were considered: inside verifier application (the verifier), inside BAP (the
...
- **Not in verifier application.** verifier application must never sign (its dependency-direction wall).
...
- The cost (a repo for a modest amount of code) buys the invariant: once the
  ...
  not a refactor.
- The adapter is small, single-purpose, and reusable by any BaseLabs edge agent
  that needs to sign application reports — "BAP produces the bytes, this adapter signs
  them, hands back the envelope."
- The dependency-direction wall (ADR 0003) holds symmetrically on both sides:
  verifier application does not depend on this adapter, and this adapter depends only on BAP.
